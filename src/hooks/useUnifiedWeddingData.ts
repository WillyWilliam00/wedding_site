...
export function useUnifiedWeddingData() {
  ...
  const activeData: UnifiedWeddingData = {
    ...
    hero: {
      ...
      image_url: sanityData.siteSettings.heroImage
        ? urlFor(sanityData.siteSettings.heroImage).url()
        : '',
    ...
    location: {
      ...
      coordinates: sanityData.location?.coordinates
        ? ([sanityData.location.coordinates.lng, sanityData.location.coordinates.lat] as [
            number,
            number,
          ])
    ...
    chiesa: {
      ...
      coordinates: sanityData.chiesa.coordinates
        ? ([sanityData.chiesa.coordinates.lng, sanityData.chiesa.coordinates.lat] as [
            number,
            number,
          ])
        : ([0, 0] as [number, number]),
      ...
      gallery:
        sanityData.chiesa.gallery?.map((image: {asset: any; alt: string}) => {
          return {
    ...
    rsvp: {
      ...
      subtitle: sanityData.contactSettings?.rsvpDeadline
        ? `Rispondi entro il ${sanityData.contactSettings.rsvpDeadline}`
        : '',
      fields: {
      ...
      foodPreferences: {
        title:
          sanityData.contactSettings?.rsvpLabels?.foodPreferencesTitle || 'Preferenze Alimentari',
